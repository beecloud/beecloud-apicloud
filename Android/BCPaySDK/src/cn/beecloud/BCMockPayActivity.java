    ...
    void initButtonBehaviors() {
        cancelView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
            	...
            	try {
        			ret.put("result_code", BCPayResult.BC_CANCEL);
        			ret.put("result_msg", BCPayResult.RESULT_CANCEL);
        			ret.put("err_detail", BCPayResult.RESULT_CANCEL);
                ...
                BCCache.executorService.execute(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (response.code == 200) {
                            ...
                            if (resultCode == 0) {
                                JSONObject succRet = new JSONObject();
                            	
                            	try {
                            		succRet.put("result_code", BCPayResult.BC_SUCC);
                            		succRet.put("result_msg", BCPayResult.RESULT_SUCCESS);
                            		succRet.put("err_detail", BCPayResult.RESULT_SUCCESS);
                        		} catch (JSONException e1) {
                        			e1.printStackTrace();
                        		}
                            	
                            	BCPay.moduleContext.success(succRet, true);
                            } else {
                                JSONObject failRet = new JSONObject();
                            	
                            	try {
                            		failRet.put("result_code", BCPayResult.BC_ERR_FAIL);
                            		failRet.put("result_msg", responseMap.get("result_msg"));
                            		failRet.put("err_detail", responseMap.get("err_detail"));
                        		} catch (JSONException e1) {
                        			e1.printStackTrace();
                        		}
                            	
                            	BCPay.moduleContext.success(failRet, true);
                            }
                        } else {
                        	JSONObject failRet = new JSONObject();
                        	
                        	try {
                        		failRet.put("result_code", BCPayResult.BC_ERR_CODE_COMMON);
                        		failRet.put("result_msg", BCPayResult.FAIL_NETWORK_ISSUE);
                        		failRet.put("err_detail", "Network Error:" + response.code + " # " + response.content);
                    		} catch (JSONException e1) {
                    			e1.printStackTrace();
                    		}
